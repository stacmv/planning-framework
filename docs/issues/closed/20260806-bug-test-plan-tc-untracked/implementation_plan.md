...
- `test/pf-test-tc-mapping-static.sh` — новый файл, Auto-проверки TC-001..TC-007; для подключения сьюта Makefile не нужен — `test:` уже перебирает `test/*.sh` (кроме `lib.sh`)
- `Makefile` — **добавлено по ходу работы, по прямому запросу владельца репозитория** (см. Task 7). К исходному дефекту отношения не имеет: исключает миграционный сьют из `make test`. Изначальная заметка «Makefile не трогать» относилась к подключению нового сьюта и остаётся верной в этом смысле — новый сьют подхватывается без правок Makefile.
...
#### Task 7: Исключить миграционный сьют из `make test` (вне исходного скоупа)

**Task Type:** build
**Mapped Test Cases:** нет — изменение конфигурации прогона, не поведения фикса
**Files:** `Makefile`

**Происхождение.** Задача добавлена в план **задним числом**, на стадии
`/pf-qa`, после того как владелец репозитория попросил об этом по ходу сессии:
«нужно выключить тесты миграции проекта, т.к. все реальные проекты уже
смигрировали». Планом изначально не предусматривалась и к дефекту Д1-Д4
отношения не имеет. Зафиксирована здесь, потому что гейт «No unrelated changes»
считает файл, изменённый но не перечисленный в плане, дефектом плана —
и справедливо: без этой записи `Makefile` в диффе ничем не объяснён.

**Implementation Notes:**
- В цикле `test:` добавлено исключение `*/converge-migrate.sh) continue ;;`
  рядом с существующим исключением `lib.sh`.
- Сьют **не удалён**: заведён таргет `test-migration`, гоняющий его по
  требованию. Уточнение владельца: «выключить не равно удалять, просто не надо
  запускать их из make test».
- Две строки в `help`; кириллицы в echo нет — конвенция Makefile.

**Acceptance Criteria:**
- [x] `make test` не запускает `converge-migrate.sh` (проверено: 0 вхождений в выводе)
- [x] `make test-migration` на чистом дереве — `193 passed, 0 failed`, 5m49s
- [x] Экономия измерена: 22m04s → 16m14s
