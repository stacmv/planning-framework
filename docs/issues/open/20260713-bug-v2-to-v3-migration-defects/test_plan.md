...
**Expected Outcome:** Тупик снят, но конвейер не ослаблен (шаги 1-5).

> **Шаги 6-7 вынесены в follow-up `20260713-bug-input-gates-answer-from-memory`.**
>
> Живой прогон показал: при **удалённом** `test_plan.md` / `implementation_plan.md` входные гейты `/pf-impl-plan` и `/pf-execute` **не сработали** — модель судила по копии документа, оставшейся в контексте, а не по диску. Инструкция была верной прозой, но не принуждала к вызову инструмента; соседний oversized-guard принуждает (`wc -l`, подсчёт заголовков) — и работает именно поэтому.
>
> **Починка входит в диф ЭТОГО issue:** раздел «Evaluating it is a MECHANICAL check, never a memory exercise» в общем определении (`pf-size-tiers`), обязательный `ls -1 docs/issues/open/<ID>/` первым действием в обоих входных гейтах, 5 статических проверок в `test/skills-static.sh` (коммит `d865899`).
>
> В follow-up уходит **не починка, а живое подтверждение**, что модель ей подчиняется. Статический тест доказывает, что принуждение записано; что оно исполняется — не докажет ни один автотест, только человек в живой сессии.
**Priority:** High
...
| TC-048 | Гейты «файл существует» дают regenerate / keep / cancel (Р9) | Manual | High | ✓ | шаги 1-5 пройдены. Шаги 6-7 (провал гейтов на удалённый документ) вынесены в follow-up 20260713-bug-input-gates-answer-from-memory; починка — в дифе этого issue (d865899) |
