...
| `scripts/update-skills.sh` | **`# shellcheck disable=SC2016`** на `:92` (намеренно одинарные кавычки в `export PATH=…`). Добавлено по итогам Task 1: shellcheck в этом репозитории **никогда фактически не прогонялся** (все прошлые `qa_report.md` фиксируют его как SKIPPED), а Р6 расширяет гейт до `scripts/*.sh test/*.sh`. Без этой строки гейт падает. То же — `scripts/install.sh:114`. Файл обязан быть в этом списке: гейт «No unrelated changes» (`.qa-workflow.md:84`) теперь сверяет диф именно с таблицами Create/Modify/Delete, поэтому незаявленный файл в дифе валит собственную QA |
...
Фикстура **`v2-with-stub`** (и только она) содержит литерал `TODO: Run /pf-` — он попадает в диф ветки; гейт Р6 (Task 1) исключает `test/` через pathspec, так что собственная QA не падает.

> **Поправка.** Ранее здесь значились обе фикстуры — `v2-project` и `v2-with-stub`. Это противоречило **TC-020 шаг 4**, который грепает issue фикстуры `v2-project` на этот литерал и ждёт **ноль** совпадений. Литерал несёт только `v2-with-stub`; `v2-project` моделирует v2-проект **до** прогона миграции, а stub создаёт именно миграция (`migrate-v2-to-v3.sh:128-141`). Вывод про pathspec от этого не меняется.
