...
- Шаблон файла шима — точно по specs.md ("Глобальный шим `pf` (AC-6)"):
  ...
  - **(P1-1) Шим намеренно НЕ передаёт `--target "$(pwd)"`.** `cli.js` (`parseTargetDir`) по умолчанию и так возвращает `process.cwd()` — а `exec node …` не меняет рабочую директорию, поэтому `process.cwd()` равен директории вызова `pf`. Если бы шим подставлял `--target "$(pwd)"`, то вызов `pf --target <другой-путь>` давал бы argv `[--target, <cwd>, --target, <другой-путь>]`, а `parseTargetDir` использует `indexOf("--target")` (first-wins) и молча игнорировал бы пользовательский аргумент. Без жёстко зашитого `--target` пользовательский `--target <path>` (если он есть) — единственный, и корректно переопределяет умолчание; bare `pf` по-прежнему работает против текущей директории. Шаблон обязан быть одинаковым во всех трёх задачах (Task 6/7/8) и в specs.md.
...
- Шаги строго по specs.md ("Однокомандный установщик фреймворка (AC-7, AC-8)"):
  ...
  3. Если `"$INSTALL_DIR/.git"` существует — детерминированное обновление: `git -C "$INSTALL_DIR" fetch origin main && git -C "$INSTALL_DIR" reset --hard origin/main`; иначе — `git clone -b main https://github.com/stacmv/planning-framework.git "$INSTALL_DIR"`. Один и тот же путь кода в обоих случаях, без интерактивного вопроса о переустановке (US-6, AC-8). Модель ветвления (P0-1, Option B): `develop` — trunk-ветка для активной разработки (фичевые ветки ответвляются от неё и сливаются обратно); `main` — release-ветка, создаваемая из `develop`, куда релизы попадают мерджем `develop`→`main`. Установщик всегда нацелен на `main`, чтобы `curl|sh` не раздавал пользователю нестабильное состояние транка посреди мержа. Явный `-b main` обязателен при клонировании. `reset --hard origin/main` вместо `pull --ff-only` выбран намеренно (P1-3): клон принадлежит установщику, его безопасно выровнять с апстримом принудительно, а `pull --ff-only` под `set -e` падает с «сырой» ошибкой git при локальных модификациях, force-push/rebase апстрима или detached HEAD — что нарушило бы гарантию AC-8 («обновление без ошибок»); детерминированный fetch+reset такой сценарий обрабатывает без сбоев.
...
- Шаги — тот же порядок, что в `install.sh` (Task 7), средствами PowerShell, по specs.md ("`scripts/install.ps1` (Windows)"):
  ...
  3. Если `Test-Path "$InstallDir\.git"` — детерминированное обновление: `git -C $InstallDir fetch origin main` затем `git -C $InstallDir reset --hard origin/main`; иначе — `git clone -b main https://github.com/stacmv/planning-framework.git $InstallDir`. Те же обоснования, что и в Task 7, шаг 3: установщик нацелен на release-ветку `main` (`develop` — trunk для разработки; см. модель ветвления P0-1 в Task 7, шаг 3), а `reset --hard` вместо `pull --ff-only` гарантирует безошибочное обновление (AC-8), не падая на локальных модификациях/force-push/detached HEAD.
  4. Установка шима: на Windows POSIX-шим неприменим — создать `~/.claude/bin/pf.cmd` с содержимым, эквивалентным `node "<FRAMEWORK_ROOT>\tools\onboarding-tui\cli.js" %*` (`<FRAMEWORK_ROOT>` = `$InstallDir`, подставляется во время установки, не вычисляется динамически). Как и в POSIX-версии (Task 6/7), `--target` явно НЕ подставляется (P1-1): `cli.js` по умолчанию работает против `process.cwd()`, а жёстко зашитый `--target "%CD%"` заставил бы `pf --target <другой-путь>` молча игнорироваться (double-`--target`, first-wins в `parseTargetDir`). Идемпотентная перезапись файла (`Set-Content`, не `Add-Content`), без резервных копий.
...
- Windows:
  ...
  ```
  > Примечание (P0-1, Option B): модель ветвления репозитория — `develop` (trunk для активной разработки) и `main` (release-ветка, создаваемая из `develop`; релизы попадают в неё мерджем `develop`→`main`). Все raw-URL установщика (`curl|sh`, `irm|iex`) и каждый `git clone -b main`/`fetch origin main`/`reset --hard origin/main` (Task 7/8, specs.md) нацелены именно на `main`, чтобы однокомандный установщик раздавал стабильное release-состояние, а не «bleeding-edge» состояние транка посреди мержа.
