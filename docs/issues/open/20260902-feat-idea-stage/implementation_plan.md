...
План разбит на 23 задачи, сгруппированные по файлу/скиллу (не по TC), в
...
проверяет, и в конце — финальный дог-фуд-прогон (Phase C). Обоснование
порядка и отступление от буквального порядка «сначала все три тест-файла,
потом хуки» — см. раздел «Dependencies» ниже.
...
Порядок задач ниже соответствует восьми группам зависимостей, заданным для
этого плана, с одним обоснованным отступлением (Phase-boundary trade-off,
описан отдельным пунктом ниже):

1. **Справочные скиллы первыми** (Tasks 1) — `pf-idea-lenses` и
...
4. **Правки `pf-close`/`pf-git`/`pf-autopilot`/`pf-check`/`pf-brd`/
   `pf-roles`/`pf-size-tiers`** (Tasks 9-13) — зависят от того, что типы
   ...
   (Task 8) — эти скиллы используют тот же способ определения TYPE.
5. **Зеркало каркаса + `converge-to-v3.sh`** (Task 14) — независимо от
   Tasks 1-13 по содержанию, но логически идёт после того, как набор
   документов idea/spike зафиксирован (`required_docs()` в
   `converge-to-v3.sh` ссылается на эти имена файлов).
6. **Документация/счётчики скиллов** (Task 15) — обязана идти **до**
   чекпоинта Task 18 (`make test`), потому что `test/docs-refs.sh`'s
   ...
   тест изменён.
7. **Два новых теста, покрывающих Tasks 1-15** (Tasks 16-17) —
   `test/pf-idea-stage-static.sh` (статический + fixture-based) и
   `test/pf-idea-templates-mirror.sh`.
8. **Front-loaded hook-сайты в 13 существующих скиллах** (Tasks 19-20) —
   ...
   specs-part1.md §3.8) — третий новый тест (Task 21) проверяет именно эти
   правки.
...
проверяет содержимое hook-сайтов (Task 20), которых до Task 20 не
...
front-loaded-static.sh` **после** hook-сайтов (Task 21), а не одним пакетом
с двумя другими тест-файлами (Tasks 16-17), которые действительно можно и
нужно написать сразу после соответствующей им функциональности (Phase A).
Дублирующая логика — та же, что уже описывает сам спек в §8's преамбуле
(«каждый TC отдельно называет свой test file») — тест-файл пишется тогда,
когда впервые может содержательно проходить, не раньше.
...
- [ ] TC-015 passes
- [ ] TC-016 passes (частично — только таблица бюджетов; полное прохождение требует Task 12)
- [ ] TC-019 passes
...
- [ ] TC-001 passes (частично)
- [ ] TC-011 passes
- [ ] TC-012 — manual, per test_plan.md
...
- **Автопилот и front-loaded (US-10b, specs-part1.md §3.5.4):** Режим 2
  ...
  вопросов (детальная правка `pf-autopilot` — Task 11; здесь фиксируется
...
- **Ветка «Сразу проект» (AC-01c) — devять шагов, specs-part1.md §3.1.1:**
  ...
  3. Записать `.pf-version` = значению `version:` из шапки **самого
     ...
     **не** `PF_VERSION` из `converge-to-v3.sh` (недоступен установленному
     ...
     (specs-part1.md §5.2; CONTRIBUTING.md-пункт — Task 14).
...
- **«Creating prompt.md» — три ветки** (specs-part1.md §3.1.3, §6.1/§6.8
  ...
  - `idea` (НОВОЕ): `doc_language` переиспользуется тем же вопросом, что и
    ...
    **Развилка по carve-out:** голая папка без каркаса PF (сценарий
    ...
    проверяется первым, Task 13). Существующий проект — role assignment
...
- [ ] TC-021 passes
- [ ] TC-033 passes (drift-guard часть — фикстура и транскрипция в Task 17)
...
- [ ] TC-022 passes
- [ ] TC-024 passes
- [ ] TC-027 passes (частично — эта задача покрывает pf-brd/pf-roles/pf-size-tiers; полная обусловленность проверяется Task 16 по всем шести изменённым общим скиллам)
...
- **`CONTRIBUTING.md`** — пункт 6 существующего чек-листа синхронизации
  ...
  `test/pf-idea-templates-mirror.sh`» (Task 17).
...
**Implementation Notes:**
- **Это задача обязана быть выполнена до Task 18 (чекпоинт `make test`)** —
...
- [ ] TC-026 passes (частично — счётчики/имена; зеркало проверяется Task 15/17)
...
- **Пункты 2-16 (§8.1)** — по одной проверке на каждый пункт списка
  ...
  (TC-026 частично — побайтовое сравнение отдельно, Task 15), `pf-check/
...
- Прогнать `make test` на текущем состоянии дерева (Tasks 1-18
  ...
  loaded-static.sh` ещё не существует на этом чекпоинте — Task 21).
...
#### Task 23: Чекпоинт Phase B/C — `make test` зелёный, финальный дог-фуд (manual TC-003), install-полировка
...
- **TC-029 (Codex compatibility review)** — при отправке документов/
  ...
  молчанием (AC-13b). Это единственный TC, требующий реально настроенного
  Codex CLI/`codex-companion.mjs` — при недоступности помечается blocked,
...
каждой границе (Tasks 19 и 23 — явные чекпоинты).
...
**Phase C — финальная верификация и полировка (Task 23).** Явная проверка,
...
Каждая фаза заканчивается зелёным `make test` (Tasks 19, 23) до перехода к
