...
**Ручные тест-кейсы удалены (2026-09-05, решение пользователя).** Пять кейсов
(`TC-003`, `TC-012`, `TC-014`, `TC-028`, `TC-029`) требовали живой агентной
сессии или внешней системы. Их **контракты уже покрыты автоматически**, и
удаление не снимает покрытия:

| Удалён | Что проверял | Чем покрыт автоматически |
|---|---|---|
| `TC-003` | живой прогон `/pf` в пустой папке и в существующем проекте | TC-002 (вопрос-развилка), TC-030 (в пустой папке создаётся только папка issue), TC-005 (четыре варианта), TC-007 (intake-батч) |
| `TC-012` | независимость персон при живом прогоне критики | TC-011 (состав персон, единый резолв актора, закрытый словарь диспозиций) |
| `TC-014` | живое переопределение допущения | TC-013 (сессия решения) плюс ассерты `CR-017` в `pf-idea-semantic-static.sh` (определение tail clean, независимость от порядка) |
| `TC-028` | живой спайк, потребовавший кода | TC-018 (скелеты и гейт записи), TC-032 (гейт пустого Run Evidence, ветка без merge) |
| `TC-029` | измерение совместимости с Codex внешним ревьюером | выполнено однократно в ходе `/pf-codereview` (раунд 1); как повторяемый тест артефакта не формулируется — зависит от внешней системы |

Не покрытым остаётся только одно: наблюдение за тем, как это делает живая
сессия агента. Bash-сьют этого проверить не может, поэтому кейс, состоящий
исключительно из такого наблюдения, ценности как тест не несёт.
