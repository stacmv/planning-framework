...
поведенческую — цепочку вызова Codex и её fallback, агрегацию замечаний двух
...
- Проверить всю цепочку обнаружения Codex (§4 specs.md) и что молчаливый
  fallback на Claude срабатывает во всех случаях недоступности, без ошибки и
  без блокировки работы пользователя.
...
- Тестовый проект под управлением Planning Framework v3 (или сам
  ...
  IMPLEMENT → QA.
- Для случаев, требующих Codex: доступ к машине, на которой можно управляемо
  включать/выключать доступность `codex` CLI и плагина `codex`
  (codex:setup/codex:rescue) — либо через отдельную тестовую машину/контейнер,
  либо через временное переименование бинаря/плагина на время теста.
...
- **Happy path**: гарда назначает ревьюеров при создании issue; документ
  ...
  этого.
- **Error/edge conditions**: Codex назначен, но недоступен на машине (три
  варианта недоступности из цепочки §4 — плагин есть и CLI не готов, плагина
  нет и пользователь отказывается его ставить, Codex недоступен в принципе);
  поле `reviewers` отсутствует в `prompt.md` (обратная совместимость);
  открыты P0/P1-замечания на попытке выйти из `pf-codereview` через «Skip and
  continue»; `/pf-test` запускается без `code_review.md` или с
  `verdict: FAIL`.
...
### TC-004: Codex недоступен в принципе — молчаливый fallback на Claude

**Description:** Ключевой сценарий из BRD §4/specs.md §4, шаг 4: Codex
назначен для артефакта, но после всех попыток (плагин недоступен, CLI не
установлен/не авторизован, пользователь отказался от установки/настройки)
реально недоступен на машине — ревью должно тихо продолжиться силами Claude,
без ошибки и без блокировки пользователя.

**Preconditions:**
- CLI `codex` не установлен и недоступен на тестовой машине (или намеренно
  сделан недоступным/неавторизованным на время теста).
- Плагин `codex` недоступен в списке скиллов сессии.
- `prompt.md` тестового issue назначает `reviewers.code: codex` (или любой
  другой ключ) единолично.

**Steps:**

| Step | Action | Expected Result |
|------|--------|-----------------|
| 1 | Запустить `pf-codereview` (или `/pf-check` для документа) на артефакте с `reviewers.<ключ>: codex` | Framework проходит цепочку §4: плагин недоступен → предлагается установить |
| 2 | На предложении установить плагин/CLI ответить отказом | Framework не завершает работу с ошибкой |
| 3 | Наблюдать, кто фактически выполняет ревью | Ревью выполняет Claude-сабагент вместо Codex |
| 4 | Проверить итоговый вывод замечаний | В выдаче явно отмечено: «Codex недоступен — ревью выполнено Claude» (или эквивалент на `doc_language` issue) |
| 5 | Проверить, что пайплайн не заблокирован | Issue продолжает движение по пайплайну (переход к следующему шагу доступен) так же, как если бы Codex не назначался вовсе |

**Test Data:**
- `docs/issues/open/<test-issue-id>/prompt.md` (`reviewers.code: codex`, без доступного codex на машине теста)

**Expected Outcome:** Ревью артефакта завершается успешно силами Claude, с
явной пометкой о fallback, без ошибок и без блокировки пайплайна.

**Priority:** Critical

---

### TC-005: Плагин `codex` не установлен, пользователь отказывается — прямой вызов `codex exec`

**Description:** Проверяет промежуточное звено цепочки §4 (шаг 3): плагин
`codex` недоступен в этой установке Claude Code, но CLI `codex` при этом
реально присутствует на машине; framework предлагает установить плагин,
пользователь отказывается — framework вызывает `codex exec` напрямую через
Bash и показывает сырой (неструктурированный) ответ отдельным блоком.

**Preconditions:**
- CLI `codex` установлен и авторизован на машине.
- Плагин `codex` НЕ установлен/недоступен в списке скиллов текущей сессии.
- `prompt.md` тестового issue назначает `reviewers.<ключ>: codex`.

**Steps:**

| Step | Action | Expected Result |
|------|--------|-----------------|
| 1 | Запустить ревью артефакта с `reviewers.<ключ>: codex` | Framework обнаруживает отсутствие плагина `codex` и спрашивает пользователя, ставить ли его |
| 2 | Ответить отказом на предложение установить плагин | Framework не устанавливает плагин и не завершает работу с ошибкой |
| 3 | Наблюдать способ вызова Codex | Вызывается `codex exec` напрямую через Bash (без обёртки плагина), с промпт-эквивалентом §5/§6 |
| 4 | Проверить итоговый вывод | Сырой ответ `codex exec` показан пользователю как есть, отдельным блоком «Codex findings (unstructured)», без попытки сопоставить severity P0/P1/P2 |

**Test Data:**
- `docs/issues/open/<test-issue-id>/prompt.md` (`reviewers.specs: codex`, плагин `codex` недоступен в сессии, но CLI `codex` установлен на машине)

**Expected Outcome:** Ревью выполнено сырым `codex exec`, результат показан
нетронутым, отдельным неструктурированным блоком, пайплайн не заблокирован.
...
- TC-004: Codex недоступен в принципе — молчаливый fallback на Claude
- TC-005: Плагин не установлен, пользователь отказывается — прямой `codex exec`
...
| TC-004 | Codex недоступен в принципе — молчаливый fallback на Claude | Manual | Critical | [ ] | |
| TC-005 | Плагин не установлен, пользователь отказывается — прямой `codex exec` | Manual | Medium | [ ] | |
