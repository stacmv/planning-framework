...
- `TD-QA-MANUAL-ROW` (found 2026-09-14 at `/pf-qa` for 20260818-improve-project-explorer-launcher-search-and-tc-scroll) — `.qa-workflow.md`'s "Every Auto TC ... is marked done" check (`grep -c '| \[ \] *|' docs/issues/open/<ISSUE-ID>/test_plan.md`) assumes a `Manual` row's Status cell is left empty, not `| [ ] |` — but the actual `test_plan.md` template (as `pf-test-plan` produces it) keeps `| [ ] |` for `Manual` rows too, so any issue with ≥1 Manual TC always shows a nonzero unprocessed-row count even when every Auto TC is green. `/pf-qa` treated this as PASS per the check item's own title (scoped to Auto TCs), verified independently against the Status Tracker. Fix: filter the grep by the `Type` column instead of counting all `| [ ] |` rows — state: `open`
