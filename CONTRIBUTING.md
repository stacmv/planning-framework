...
2. **Bump `PF_VERSION`** in `scripts/converge-to-v3.sh`.
...
6. **On a major/minor bump only**, also update the major.minor stamp `**Framework Version:** X.Y` wherever it's checked/displayed: `PLANNING.md`, `CLAUDE.md`, `docs/planning/FRAMEWORK.md`, `docs/planning/QUICKSTART.md`, `docs/planning/templates/config/{CLAUDE.md,PLANNING.md}`, `docs/planning/templates/README.md`, and `skills/pf/SKILL.md`'s `version:` field (the second source `.pf-version` is written from, for projects scaffolded directly by `/pf` without the framework repo available). `tools/onboarding-tui/lib/detect.js`'s `V3_VERSION_RE` only matches major version `3` — a major bump past `3.x` requires updating that regex too, or detection of the new version breaks for every consumer project.

   **Any edit to `docs/planning/templates/` must be mirrored byte-for-byte into `skills/pf/templates/project/` (and vice versa)** — the latter is the copy an installed `pf` skill can scaffold a fresh project from when the framework repository itself is unavailable. Checked by `test/pf-idea-templates-mirror.sh`.
...
8. **Run `make test`** — several tests assert the current version literal (`test/lib.sh`, `test/converge-normalize.sh`, `test/docs-refs.sh`); a real version bump must update those assertions too, in the same commit.
