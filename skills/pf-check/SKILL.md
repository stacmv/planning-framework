...
2. **Plugin available.**
   ...
   b. Once `codex:setup` reports the CLI is ready (installed and authenticated), run Codex through the plugin's `codex-companion.mjs` script in foreground/blocking mode (never `--background` — the calling skill needs the result before it can present the gate), in the **form matching the review target**:
      - **Document target — this skill (`pf-check`), and the document stages of `pf-dev-docs`/`pf-user-docs`.** Path-based, no diff, no branch:
        ```
        node "${CLAUDE_PLUGIN_ROOT}/scripts/codex-companion.mjs" task "<review brief>" --json
        ```
