...
3. **Plugin not available.** Ask the user (`AskUserQuestion`) whether to install the `codex` plugin now. If they agree, tell them how (the Claude Code plugin marketplace) and stop this review for now — re-run this skill once it's installed, which resumes at step 1. If they decline, continue to step 4.
