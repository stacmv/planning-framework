...
2. Resolve the target issue:
   ...
   - **No ID and several open issues** — autopilot runs unattended (see Step 2, item 2): do not call `AskUserQuestion`. Fall back immediately to the **newest by the `YYYYMMDD` date in the folder name**, and record the fallback in that issue's `session-log.md` marked `[autopilot default]`. State the pinned issue in the run's first report line either way.
...
2. Autopilot runs unattended — nobody is necessarily there to answer. Questions that would normally go through `AskUserQuestion` (BRD/spec content Q&A, overwrite-or-keep output gates) are **not asked**: `AskUserQuestion` does not time out, so calling it here blocks the run indefinitely (observed: 24 minutes and ~7 hours stalled in one 2026-09-08 run) instead of degrading gracefully. Take the recommended/documented option immediately and record every assumed default in the issue's `session-log.md` marked `[autopilot default]`. The `/pf-check` review gate ("How would you like to proceed?") is resolved the same way, non-interactively, by pf-check's Autopilot mode (item 1). This does not apply to the final human gate (item 8 below) — that one has no safe default, so it stops the run instead of guessing.
...
- Autopilot does not pick or create issues on its own. The one exception is the unattended fallback in Step 0 (several open issues → the newest by date, no question asked), and it is logged as `[autopilot default]`.
