...
**Recovering from a failure inside this phase — what actually happens, and its limits.** By the time this phase runs, Phase 4 has already checked out PARENT-BRANCH and merged, so a failure here leaves you **on PARENT-BRANCH** with an uncommitted `docs/planning/test-plan.md`. A re-run of `/pf-close` therefore does **not** resume by itself: Phase 0's third check requires the current branch to be `issue/ISSUE-ID` and stops with "Switch to the issue branch first". The recovery is:

1. `git checkout issue/ISSUE-ID` — the partially written `docs/planning/test-plan.md` follows into the issue branch (it is either untracked or identical on both branches, so it does not conflict).
2. Re-run `/pf-close`. Phase 2 sees a non-empty `git status` and commits that partial file as `chore: pre-close cleanup [ISSUE-ID]` **on the issue branch** — not in Phase 8's archive commit as it would have been on a clean run. This is harmless but worth knowing: it means Phase 4 then performs a **real second merge commit**, not a no-op, because the issue branch has gained a commit.
3. This phase re-runs and finishes the work. No row is duplicated, because identity is the pair (`ISSUE-ID`, `TC-NNN`) (step 3 below) and the numbering rule takes the maximum of the counter and the table (step 4 below) — the two rules together make the re-run idempotent even though the surrounding phases are not.

Making the surrounding phases resume seamlessly would mean changing Phase 0's branch check or Phase 2's cleanup semantics, both of which are pre-existing `/pf-close` behavior outside this phase's remit. The limitation is documented here rather than worked around.
...
5. **Area.** Derive it from the file list of the merge commit Phase 4 just created: run `git diff --name-only HEAD^1 HEAD^2` (`HEAD` is the fresh merge commit on PARENT-BRANCH that Phase 4 just made; `HEAD^1` is the parent-branch tip before the merge, `HEAD^2` is the tip of `issue/ISSUE-ID`). **Do not use the three-dot form** `git diff --name-only PARENT-BRANCH...issue/ISSUE-ID` — after Phase 4's `--no-ff` merge, the merge-base of the two branches has become the tip of `issue/ISSUE-ID` itself, so a three-dot diff at this point always returns an empty file list (measured on a real close: 0 files vs. 15 files for the `HEAD^1 HEAD^2` form on the same repository state). Exclude any paths under `docs/issues/` **and any under `docs/planning/`** from the result — neither the issue's own planning documents nor the framework's own bookkeeping files (`session-log.md`, `decisions.md`, this very `test-plan.md`) describe a product subsystem. Excluding `docs/planning/` also keeps the recovery path from mislabelling: on the second merge described in "Recovering from a failure inside this phase" above, `HEAD^1 HEAD^2` can differ by nothing but `docs/planning/test-plan.md`, which would otherwise yield the meaningless `Area: docs` for every row promoted on that run. Of the remaining paths, take the second path segment for anything under `skills/` or `tools/` (e.g. `skills/pf-close/…` → `pf-close`), and the first segment for everything else (e.g. `scripts/…` → `scripts`). The segment with the most files wins; if nothing remains after the exclusions, use `general`.
