...
5. **Area.** Derive it from the file list of the merge commit Phase 4 just created: run `git diff --name-only HEAD^1 HEAD^2` (`HEAD` is the fresh merge commit on PARENT-BRANCH that Phase 4 just made; `HEAD^1` is the parent-branch tip before the merge, `HEAD^2` is the tip of `issue/ISSUE-ID`). **Do not use the three-dot form** `git diff --name-only PARENT-BRANCH...issue/ISSUE-ID` — after Phase 4's `--no-ff` merge, the merge-base of the two branches has become the tip of `issue/ISSUE-ID` itself, so a three-dot diff at this point always returns an empty file list (measured on a real close: 0 files vs. 15 files for the `HEAD^1 HEAD^2` form on the same repository state). Exclude any paths under `docs/issues/`, **and the bookkeeping files sitting directly in `docs/planning/` — that is, `docs/planning/*.md` at the top level only** (`session-log.md`, `decisions.md`, `implementation-plan.md`, and this very `test-plan.md`). None of those describes a product subsystem, and excluding them keeps a row from being labelled the meaningless `Area: docs` when the merge diff happens to contain nothing else.

   **Do not exclude subdirectories of `docs/planning/`** — in particular not `docs/planning/templates/`, which holds real product artifacts of this framework. An issue whose whole change is editing a template (the issue that added `docs/planning/templates/global/test-plan.md` did exactly that) must still get a meaningful `Area`, so the exclusion is deliberately the top-level files and not the whole tree.
