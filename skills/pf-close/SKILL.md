...
**Why discarding is required and not merely tidier.** Keeping the partial write breaks the recovery in two separate ways, both measured:
...
- **`Area` comes out wrong** even where the checkout does succeed. A preserved partial write makes Phase 2 commit it as `chore: pre-close cleanup [ISSUE-ID]` on the issue branch, which gives the branch a new commit, so Phase 4's re-run creates a **second** merge commit instead of a no-op. Step 5's `git diff --name-only HEAD^1 HEAD^2` then describes that second merge — bookkeeping only — instead of the issue's real file set, and every row promoted on the retry gets a meaningless `Area`.

Discarding avoids both: the tree stays clean, Phase 2 has nothing to commit, Phase 4's re-merge really is `Already up to date` with `HEAD` still pointing at the original merge commit, and step 5 therefore computes `Area` from the original issue diff exactly as on a first run.
