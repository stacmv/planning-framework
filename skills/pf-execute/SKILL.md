...
2. **`Task Type: docs` guard — check before creating any tasks.** If any task in `implementation_plan.md` carries `**Task Type:** docs`, stop here and surface it to the user as an unsupported task type: `docs` is reserved and not dispatched by this issue's implementation (matching `<PF_SKILL_ROOT>/pf-impl-plan/SKILL.md`'s own "reserved... not used for delegation in this issue" framing for this value). Do not call `TaskCreate` for any task yet — catching this at plan-parse time, before any wave starts, avoids stranding an already-running wave on an undispatchable task (see Phase 2, "Execution Strategy" point 2 for what would otherwise happen at dispatch time).
...
4. **Create each task** with:
   ...
   - **`Task Type`** — carry the task's `**Task Type:** code | tests`
     ...
     `<PF_SKILL_ROOT>/pf-impl-plan/SKILL.md`; `docs` was already ruled out
...
2. **Resolve each task's write actor before dispatching it.** For `size_tier` small/medium/large, each task's `Task Type` (`code` | `tests`) was already carried into the task's own description/metadata at creation time (Phase 1, "Create Tasks from Implementation Plan") — read it from there, not by re-reading `implementation_plan.md` from scratch for each task.
   ...
   Once the task's effective `Task Type` is known (carried value, or the missing-field fallback above), resolve role — same "resolve role" pattern used elsewhere for whole documents, applied here per task instead — per `<PF_SKILL_ROOT>/pf-roles/SKILL.md` §4, for `code` (or, when the task's `Task Type` is `tests`, for `tests`) to get that task's `write` actor:
   ...
   - **`write != claude`** (in this issue, only `codex`) — in Codex runtime the orchestrating session performs the task directly according to `<PF_SKILL_ROOT>/pf-roles/SKILL.md` §7; it does not call a Claude plugin path. In Claude runtime use the configured Codex adapter. See "For Each Task — `write != claude` (delegated actor)" below (TC-012).
3. **Run each `write: claude` task in its own sub-agent** - This keeps context usage low (~18% vs ~56%)
4. **Within a wave, run `write != claude` (Codex-delegated) tasks sequentially, after every `write: claude` sub-agent in that wave has finished, and before that wave's commit — never concurrently with other writers.** In Codex runtime the current session performs these tasks directly; in Claude runtime the configured Codex adapter is used. This ordering prevents working-tree races and applies whether or not sub-agent task tools are available.
5. **Process waves sequentially** - Wave N+1 starts only after Wave N completes
6. **Commit and push after each wave, not inside sub-agents or delegated-actor calls.** Claude sub-agents in a wave run concurrently and must NOT run `git commit` themselves (concurrent commits on the same branch race and corrupt each other); Codex-delegated tasks (run sequentially per point 4 above) must not commit themselves either — same reason, one commit per wave. Once every task in a wave is confirmed complete, the orchestrator runs the shared commit & push procedure in `<PF_SKILL_ROOT>/pf-git/SKILL.md` ("Stage commit & push") for that wave, on the issue branch. How "confirmed complete" is judged differs by task kind, and the two channels are not symmetric:
...
3. In Codex runtime it performs the resolved Codex task directly and rereads the target files. In Claude runtime it calls the resolved actor's write-invocator per `<PF_SKILL_ROOT>/pf-roles/SKILL.md` §7, synchronously for a small task or asynchronously for a multi-file task.
