...
- `reviewers.<key>: self` → the current runtime/master actor; `peer` → the
  other installed supported actor, falling back to the runtime/master actor
  when that peer is unavailable. These runtime-relative values are normalized
  before writing the explicit `roles:` entry.
