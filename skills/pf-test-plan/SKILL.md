...
- If `write == claude` — unchanged: **do not read these documents or draft the plan yourself** (beyond the mechanical count above). Run the Availability check (`~/.claude/skills/pf-roles/SKILL.md` §11), then dispatch a single sub-agent (Agent tool, default/general-purpose type — no need for a fork, `model: <resolved tier>` per §9) to do the reading, drafting, and writing — this sub-agent-dispatch mechanism is unchanged at every tier, including trivial: a sub-agent is still dispatched for trivial-tier test plans, only the source document and target counts differ. Give it the issue ID, `size_tier`, which source document(s) to read, and the full structure below (Steps 1-5).
- If `write != claude` (in this issue, only `codex`) — run the Availability check (`~/.claude/skills/pf-roles/SKILL.md` §11). This skill has no `AskUserQuestion` clarifying loop of its own to run (the test plan is derived mechanically from the predecessor documents, the same way the sub-agent path already does it). Instead of dispatching a Claude sub-agent, delegate the write to the resolved actor's write-invocator per `~/.claude/skills/pf-roles/SKILL.md` §7, targeting `docs/issues/open/[ACTIVE-ISSUE-ID]/test_plan.md` with a single prompt carrying the same instructions the sub-agent would otherwise receive: the issue ID, `size_tier`, which source document(s) to read (by path — the actor reads them itself, the same "do not inline document content" boundary the mechanical-count guard above already observes), `doc_language`, and the full structure below (Steps 1-5). A from-scratch pipeline document is §7's asynchronous case.
