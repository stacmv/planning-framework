...
- `agents.yml`'s default schema (`pf-roles/SKILL.md` §2) gains a `human: { kind: human, inbox: <queue-id> }` actor — `roles.<key>.write`/`review` can now resolve to a real person instead of always to `claude`/`codex`. `roles.<key>` also gains an optional `mode: blocking | non-blocking` field (default `non-blocking`, distinct from `review.mode: parallel | sequential` — do not confuse the two), and the resolver no longer hard-stops on `kind: human`; it returns a structured `{ kind: "human", ... }` result instead, leaving handling to the caller. `/pf-close` Phase 0 gained a prerequisite check: every pipeline key resolving to `kind: human` (via `write` or any `review[]` entry) must carry a `[human-task done] <key> @ <ts> content-hash=<sha256>` marker in `session-log.md`, and a stale (content-hash mismatch) marker counts as not-done. `/pf-autopilot`'s work loop deletes its own cron schedule and ends the run with a report — rather than retrying uselessly on the next resume — when `/pf-close` stops on an unresolved human-task

### Fixed
- `make test` runtime: 194 s → ~11-19 s on Linux, with no check removed. The dominant cost was a single glob substitution in the test harness — `${content/$m_search/$m_replace}` in `test/pf-idea-semantic-mutations.sh` treats its search text as a **pattern**, and manifest rows quoting markdown bold (`**`) sent bash backtracking over 68 KB of file content: 57.9 s for one row, 137 s of that suite's 145 s, 84% of the whole run. Now a literal `awk index()` replace, guarded by `test/safety-audit.sh` step 8. Alongside it: `snapshot_tree` hashes in one pass, `shellcheck` moved out of `test/docs-refs.sh` into a new `make lint` target (which fails loudly when shellcheck is absent instead of passing silently), suites run in parallel via `xargs -n 1 -P` with a private log and exit status per suite, and `converge-to-v3.sh`'s `t6_mirror_templates`/`t7_skills` batch their copies. Suite count unchanged at 27; assertions 1149 → 1150 on develop (1151 on the issue branch, where one qa-gates step only runs)
- `scripts/converge-to-v3.sh` phase 6 discarded `t6_mirror_templates`' exit status, so a missing templates source or a failed mirror copy still exited 0 with a success report. The status now reaches `EXIT_CODE`
- `test/lib.sh`'s `pf_repo_copy` refuses to run where `.git` is a file (a linked git worktree) instead of copying the pointer: git commands "inside the copy" were mutating the real repository, and the suites' deliberate probe commits landed on the branch under test while `assert_repo_untouched` stayed green. Guarded by `test/safety-audit.sh` step 7. Consequence to know about: the test suite cannot be run from a worktree — see `docs/planning/tech-debt.md`
...
### Changed
- Project Explorer's document-status surface (`tools/manual-test-ui`): a missing document on a **closed** issue now reads as `not_applicable` instead of `missing` — a closed issue is an archive, and a document absent there is not a defect demanding attention. Shipped as unsolicited scope in the CR-016/CR-017 fix (Task 35) and consciously accepted by the owner (2026-09-07); open issues and legitimately `skip`-resolved keys keep their existing semantics
